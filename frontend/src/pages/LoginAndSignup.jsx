...
const LoginAndSignUp = () => {
    ...
    const handleSubmit = async(e) => {
        e.preventDefault()
        try{
          const res = await axiosInstance.post(`/auth/${state==='login'?'login':'signup'}`,state==='login'?{email:formData.email,password:formData.password}:{fullName:formData.name,email:formData.email,password:formData.password})
          console.log(res);
    ...
    return (
        <>
        <div className="min-h-screen bg-[#0F1117] flex justify-center items-center px-4"> 
            <form
                onSubmit={handleSubmit}
                className="w-full sm:w-95 text-center bg-white/6 border border-white/10 rounded-2xl px-8">
